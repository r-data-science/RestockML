...
#' @param CLEAN Default is False. Whether to delete the session output directory. Useful for debuging
...
server_prm <- function(input, output, session) {

  .colors <- get_app_colors()

  # create the waiter
  w <- Waiter$new(
    html = tagList(spin_pulsar(), br(), "Initializing..."),
    color = .colors$bg
  )
  ...
  ## This dataset is reactively filtered by the user and
  ## contains SKUs to run recs
  ...
  r_org_uuid <- reactive({
    as.data.table(rec_args())[
      get("org") == req(input$`filters-org`),
      get("org_uuid")[1]
    ]
  })
  r_store_uuid <- reactive({
    as.data.table(rec_args())[
      get("store") == req(input$`filters-store`),
      get("store_uuid")[1]
    ]
  })
  r_skus <- reactive({
    as.data.table(rec_args())[
      get("org_uuid") == r_org_uuid() &
        get("store_uuid") == r_store_uuid(),
      get("product_sku")
    ]
  })
  ...
  observeEvent(input$btn_post, {
    w$show()
    w$update(html = tagList(spin_pulsar(), br(), "Gathering Outputs..."))

    ## This will read from the temp outputs and construct
    ## the data needed to population report
